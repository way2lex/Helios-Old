...
def getleechinfo(from_user):
    ...
    if (
        ...
        ltype = "ᴅᴏᴄᴜᴍᴇɴᴛ"
        buttons.sbutton("sᴇɴs as ᴍᴇᴅɪᴀ", f"leechset {user_id} med")
    else:
        ltype = "ᴍᴇᴅɪᴀ"
        buttons.sbutton("sᴇɴᴅ as ᴅᴏᴄᴜᴍᴇɴᴛ", f"leechset {user_id} doc")
    ...
    if AUTO_DELETE_MESSAGE_DURATION == -1:
        buttons.sbutton("ᴄʟᴏsᴇ", f"leechset {user_id} close")
...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer(text="ʏᴏᴜʀ ғɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟɪᴠᴇʀᴇᴅ ᴀs ᴅᴏᴄᴜᴍᴇɴᴛ!", show_alert=True)
        ...
        query.answer(text="ʏᴏᴜʀ ғɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟɪᴠᴇʀᴇᴅ ᴀs ᴍᴇᴅɪᴀ!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "thumb":
        ...
        if ospath.lexists(path):
            ...
            query.answer(text="ᴏʟᴅ sᴇᴛᴛɪɴɢs", show_alert=True)
    else:
        query.answer()
        try:
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        sendMessage("ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴘʜᴏᴛᴏ ᴛᴏ ᴀʟsᴀᴠᴇ ᴄᴜsᴛᴏᴍ ᴛʜᴜᴍʙɴᴀɪʟ.", context.bot, update.message)
