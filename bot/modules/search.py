...
def torser(update, context):
    ...
    if SEARCH_API_LINK is  None and SEARCH_PLUGINS is None:
        ...
        buttons.sbutton('ᴀᴘɪ', f"torser {user_id} apisearch")
        buttons.sbutton('ᴘʟᴜɢɪɴs', f"torser {user_id} plugin")
        buttons.sbutton("ᴄᴀɴᴄᴇʟ", f"torser {user_id} cancel")
        button = InlineKeyboardMarkup(buttons.build_menu(2))
        sendMarkup('ᴄʜᴏᴏsᴇ ᴛᴏᴏʟ ᴛᴏ sᴇᴀʀᴄʜ:', context.bot, update.message, button)
    elif SEARCH_API_LINK is not None and SEARCH_PLUGINS is None:
        button = _api_buttons(user_id, "apisearch")
        sendMarkup('ᴄʜᴏᴏsᴇ sɪᴛᴇ ᴛᴏ sᴇᴀʀᴄʜ:', context.bot, update.message, button)
...
def _getResult(search_results, key, message, method):
    ...
    path = [telegraph.create_page(
                title='sᴘʏ x ᴍɪʀʀᴏʀ ᴛᴏʀʀᴇɴᴛ sᴇᴀʀᴄʜ',
                content=content
